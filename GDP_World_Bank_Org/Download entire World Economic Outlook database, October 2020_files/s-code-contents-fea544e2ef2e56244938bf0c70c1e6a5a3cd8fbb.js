...
var mediaPlayerName = "Brightcove Smart Player";
 //Required hard code player name here.
  
	player = brightcove.getExperience(players.pop());
	
    
   // console.log("hey");
	//modCon = player.getModule(brightcove.modules.APIModules.CONTENT);
...
//console.log(modCon);
	...
	modVP.addEventListener("mediaPlay", function (evt1) {
    console.log("lll");
	window.aaaa=evt1;
	mediaLength = evt1.duration; //Required video duration
	mediaOffset = Math.floor(evt1.position); //Required video position
	mediaID = (evt1.media.id).toString(); //Required video id
	mediaFriendly = evt1.media.displayName; //Required video title
  ...
  modVP.addEventListener("mediaSeek", function (evt4) {
    console.log("seek");
    window.aaa3=evt4;
	mediaOffset = Math.floor(evt4.position);
	s.Media.stop(mediaName,mediaOffset);
});
	modVP.addEventListener("mediaSTOP", function (evt2) {
    console.log("stop");
    window.aaa1=evt2;
	mediaOffset = Math.floor(evt2.position);
	...
	modVP.addEventListener("mediaProgress", function (evt3) {
    console.log("hhhh");
    window.aaa2=evt3;
    console.log("xxxx2");
    try{
...
s.Media.trackVars="events,prop51,eVar51,contextData.bc_tags,contextData.bc_refid,contextData.bc_player,contextData.bc_playertype,contextData.bc_playlist";
s.Media.trackEvents="event51,event52,event53,event54,event55,event56";
s.Media.trackMilestones="20,50,80";
s.Media.segmentByMilestones = true;
s.Media.trackUsingContextData = true;
s.Media.contextDataMapping = {
  "a.media.name":"eVar51,prop51",
  "a.media.timePlayed":"event56",
  "a.media.view":"event51",
  "a.media.complete":"event55",
  "a.media.milestones":{
     20:"event52",
     50:"event53",
     80:"event54"
